            ...
            if not self.training:  # inference
                ...
                if self.inplace:
                    ...
                    if self.nkpt != 0:
                        ...
                        x_kpt[..., 1::3] = (x_kpt[..., 1::3] * 2. - 0.5 + kpt_grid_x.repeat(1,1,1,1,self.nkpt)) * self.stride[i]  # xy
                        x_kpt[..., 1::3] = (x_kpt[..., 1::3] * 2. - 0.5 + kpt_grid_y.repeat(1,1,1,1,self.nkpt)) * self.stride[i]  # xy
                        x_kpt[..., 2::3] = x_kpt[..., 2::3].sigmoid()
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        for m in self.model[-5:]:
            if isinstance(m, Detect) or isinstance(m, IDetect) or isinstance(m, IKeypoint) or isinstance(m, IDetectHead):
                s = 256  # 2x min stride
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        elif m in [Detect, IDetect, IKeypoint, IDetectHead]:
