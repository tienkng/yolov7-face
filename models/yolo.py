...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None, headlayers=None, export=False):  # model, input channels, number of classes
        ...
        for m in self.model[-5:]:
            if self.headlayers.check(m):
                # format_list.append(m.__class__.__name__)
                s = 256  # 2x min stride
